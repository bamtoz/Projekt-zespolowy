%for indeks = 1:35
   ...
   % Logarytm
    ...
    destinationFolder = strcat(cd,'\SplittedR'); %Tworzy folder o nazwie splitted
    ...
    for r = 1 : wholeBlockRows
       for c = 1 : wholeBlockCols
           ...
           imwrite(splittedImage, fullfile(cd, 'SplittedR', sprintf('output%d.tif',saveIndex)));
           ds = augmentedImageDatastore(imageSizeR, splittedImage,'ColorPreprocessing','gray2rgb');
           imageFeatures = activations(netR,ds,featureLayerR,'MiniBatchSize',32,'OutputAs','columns');
           label = predict(classifierR, imageFeatures,'ObservationsIn','columns');
    ...
    if(srednia>1.5)
           sprintf('The loaded %d image belongs to Normal class',indeks)
    else
           sprintf('The loaded %d image belongs to Cancer class',indeks)
    end
